...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(a2_local_M) == (NULL)) && !rtmGetStopRequested
    (a2_local_M);
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(a2_local_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(a2_local_M, true);
      }

      if (rtmGetStopRequested(a2_local_M) == true) {
    ...
    stopRequested = !((rtmGetErrorStatus(a2_local_M) == (NULL)) &&
                      !rtmGetStopRequested(a2_local_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(a2_local_M, "stopping the model");
  runModel = 0;
}
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(a2_local_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(a2_local_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(a2_local_M, 0);
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(a2_local_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(a2_local_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(a2_local_M, true);
    }
  }
