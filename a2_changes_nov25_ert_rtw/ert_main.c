...
#include "a2_changes_nov25_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(a2_changes_nov25_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    a2_changes_nov25_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(a2_changes_nov25_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(a2_changes_nov25_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(a2_changes_nov25_M, 0);

  /* Initialize model */
  a2_changes_nov25_initialize();
