...
def AOO_selections():
    ...
    #ranges for inputs, increments for those ranges, label, default value 
    AOO_screen.addInputBox([30,50,90,175],[5,1,5],"LRL (ppm) ",60) 
    AOO_screen.addInputBox([50,175],[5],"URL (ppm) ",120)
    AOO_screen.addInputBox([0.5,3.2,3.5,7],[0.1,0.5,0.5],"Pulse Amplitude (V) ",3.5)
    AOO_screen.addInputBox([0.05,0.1,1.9],[0.05,0.1],"Pulse Width (ms) ",0.4)
...
def VOO_selections(): ##literally the same as AOO except with some varible names swapped
   ...
   ### FROM HERE TO NEXT COMMENT IS CRITICAL URL ###
    ...
    def save_sel():
        ...
        if(get_LRL() and get_URL() and get_VPulseAmp() and get_VPulseWidth()):
            LRL = LRL_menu.get()
            URL = URL_menu.get()
            ...
            warningLabel2 = Label(VOO_screen, text="")
            warningLabel2.pack(side=TOP, padx=0, pady=0, anchor='n')
            warningLabel.config(bg="yellow", text="Inserted invalid value in one of your selections. Data has not been saved. \n Please check your inputs and try again. ")   
   ...
   # warningLabel.place(x=warningLabelX,y=warningLabelY)
    ...
    def save_sel():
        if i == 0:
            ...
            #print(username_info) #works for register but not edit
            file = open(username_info, 'a')
        RSok = get_RS() #idk it would just sometimes skip get rs so i am forcing it to do it here

        if(get_LRL() and get_URL() and get_APulseAmp() and get_HRL() and get_APulseWidth() and get_ASensitivity() and get_ARP() and get_PVARP() and RSok):
            LRL = LRL_menu.get()
            URL = URL_menu.get()
...
def VVI_selections():
   ...
   # warningLabel.place(x=warningLabelX,y=warningLabelY)
    ...
    def save_sel():
        ...
        URLOk = get_URL()
        APOK = get_VPulseAmp()
        APWOk = get_VPulseWidth()
        ASense = get_VSensitivity()
        ARPOK = get_VRP()
        HRLOk = get_HRL()
        RSok = get_RS() #idk it would just sometimes skip get rs so i am forcing it to do it here
        if(get_LRL() and get_URL() and get_VPulseAmp() and get_HRL() and get_VPulseWidth() and get_VSensitivity() and get_VRP() and get_HRL() and RSok):
            LRL = LRL_menu.get()
            URL = URL_menu.get()
            AA = VPulseAmp_menu.get()
            APW = VPulseWidth_menu.get()
            AS = VSensitivity_menu.get()
            ARP = VRP_menu.get()
            HRL = HRL_menu.get()
            RS = RS_menu.get()
            file.write(LRL + '\n')
